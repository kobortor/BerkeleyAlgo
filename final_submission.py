...
if __name__ == "__main__":
    ...
    for idx, row in df.iterrows():
        if idx % 1000 == 0:
            print(f"{idx} / {len(df)}")
        pred_edge = int(algorithm(row.pred_edge, row.stk, row.xchg, row.px, row.sz, row.dir, row.cp_naive, row.sm_naive) > 0)
        # result.loc[idx] = {'pred edge': pred_edge}
        results[idx] = pred_edge

    df = pd.DataFrame.from_dict(results, orient='index', columns=['trade'])
